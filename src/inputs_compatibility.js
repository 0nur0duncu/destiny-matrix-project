...
// Add event listener for compatibility analysis
document.addEventListener('DOMContentLoaded', function() {
    const analyzeBtn = document.getElementById('analyze-compatibility-btn');
    const loadingDiv = document.getElementById('compatibility-loading');
    const resultDiv = document.getElementById('compatibility-result');
    const resultText = document.getElementById('compatibility-text');

    if (analyzeBtn) {
        analyzeBtn.addEventListener('click', async function() {
            if (!person || !secondPerson || !compatibilityMatrix) {
                alert('Please create your compatibility matrix first by entering both birth dates.');
                return;
            }
            ...
            const compatibilityData = captureCompatibilityData(person, secondPerson, compatibilityMatrix);
            const analysis = await analyzeMatrix(compatibilityData, 'compatibility');
