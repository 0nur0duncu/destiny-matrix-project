...
async function analyzeMatrix(matrixData, analysisType = 'personal') {
  try {
    const response = await fetch('http://localhost:8000/analyze-matrix', {
      method: 'POST',
      headers: {
    ...
    return 'Sorry, there was an error analyzing your matrix. Please try again later.';
  }
}

function captureMatrixData(person) {
  return {
...
function captureCompatibilityData(person1, person2, compatibilityMatrix) {
  return {
    person1: person1,
    person2: person2,
...
// Add event listener for personal matrix analysis
document.addEventListener('DOMContentLoaded', function() {
  const analyzeBtn = document.getElementById('analyze-matrix-btn');
  ...
  if (analyzeBtn) {
    analyzeBtn.addEventListener('click', async function() {
      if (!person || !person.points) {
        alert('Please create your matrix first by entering your birth date and name.');
        return;
      }
      ...
      const matrixData = captureMatrixData(person);
      const analysis = await analyzeMatrix(matrixData, 'personal');
